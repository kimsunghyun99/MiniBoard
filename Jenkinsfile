    ...
    stages {
        ...
        stage("Docker Image Build") {
           steps {
               ...
               sh "docker build -t hjj6611/apache2_smboard:${BUILD_NUMBER} ./docker/apache2/"
               sh "docker build -t hjj6611/smboard_smboard:${BUILD_NUMBER} ./docker/smboard/"
               sh "docker build -t hjj6611/mariadb_smboard:${BUILD_NUMBER} ./docker/mariadb/"
           }
        }
        stage("Docker Image Push") {
           steps {
               sh "docker push hjj6611/apache2_smboard:${BUILD_NUMBER}"
               sh "docker push hjj6611/smboard_smboard:${BUILD_NUMBER}" 
               sh "docker push hjj6611/mariadb_smboard:${BUILD_NUMBER}" 
           } 
        }
        stage("Docker Image Clean up") {
           steps {
               sh "docker image rm hjj6611/apache2_smboard:${BUILD_NUMBER}" 
               sh "docker image rm hjj6611/smboard_smboard:${BUILD_NUMBER}" 
               sh "docker image rm hjj6611/mariadb_smboard:${BUILD_NUMBER}" 
           }
        }
